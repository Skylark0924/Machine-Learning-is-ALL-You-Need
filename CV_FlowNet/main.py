...
if __name__ == '__main__':
    ...
    tools.add_arguments_for_module(parser, losses, argument_for_class='loss', default='FlowLoss')
    ...
    with tools.TimerBlock("Parsing Arguments") as block:
        ...
        # print('losses', tools.module_to_dict(losses))
    ...
    with tools.TimerBlock("Initializing Datasets") as block:
        ...
        if exists(args.training_dataset_root):
            # print('args', args.training_dataset_class)
        ...
        if exists(args.validation_dataset_root):
            ...
            validation_dataset = args.validation_dataset_class(args, False, **tools.kwargs_from_args(args, 'validation_dataset'))
            ...
            validation_loader = DataLoader(validation_dataset, batch_size=1, shuffle=False, **gpuargs)
        ...
        if exists(args.inference_dataset_root):
            ...
            # inference_dataset = args.inference_dataset_class(None, False, **tools.kwargs_from_args(args, 'inference_dataset'))
            ...
            # inference_loader = DataLoader(inference_dataset, batch_size=8, shuffle=False, **inf_gpuargs)
    ...
    with tools.TimerBlock("Building {} model".format(args.model)) as block:
        def calc_EPE(input_flow, target_flow):
            epe_value = torch.norm(target_flow-input_flow,p=2,dim=1).mean()
            return [epe_value]
        ...
        class ModelAndLoss(nn.Module):
            def __init__(self, args):
                ...
                print('### Print Model Flops ###')
                print_model_parm_flops(self.model)
                print('### end of Flops Calc ###')
                
            def forward(self, data, target, validation=False, inference=False ):
                output = self.model(data)

                # loss_values = self.loss(output, target)
                # loss_values = self.loss(output[0], target)

                if not inference :
                    if validation:
                        loss_values = calc_EPE(output, target)
                    else:
                        loss_values = self.loss(output, target)
                    return loss_values
                else :
    ...
    def train(args, epoch, start_iteration, data_loader, model, optimizer, logger, is_validate=False, offset=0):
        ...
        for batch_idx, (data, target) in enumerate(data_loader):
            if is_validate:
                L = len(data_loader)
                if batch_idx % (L//2) == 0 or (batch_idx == L-1):
                    print('Eval-iters: %d/%d' % (batch_idx, L))
            data, target = [Variable(d) for d in data], [Variable(t) for t in target]
            # if is_validate:
                # assert 1<0, (data[0].shape, target[0].shape, len(data))
            if args.cuda and args.number_gpus == 1:
                data, target = [d.cuda(non_blocking=True) for d in data], [t.cuda(non_blocking=True) for t in target]

            optimizer.zero_grad() if not is_validate else None
            losses = model(data[0], target[0], validation=is_validate)
            losses = [torch.mean(loss_value) for loss_value in losses] 
            # loss_val = losses[0] # Collect first loss for weight update
            # loss_val = losses[1] # Collect EPE loss
            loss_val = losses[0] # Collect EPE loss
            # assert 1<0, losses
            ...
            if ((((global_iteration + 1) % args.log_frequency) == 0 and not is_validate) or
                ...
                # print('loss_label', loss_labels, all_losses)
    ...
    def inference(args, epoch, data_loader, model, offset=0):
        ...
        for batch_idx, (data, target) in enumerate(tqdm(data_loader)):
            if args.cuda:
                data, target = [d.cuda(non_blocking=True) for d in data], [t.cuda(non_blocking=True) for t in target]
            data, target = [Variable(d) for d in data], [Variable(t) for t in target]
            # assert 1<0, (data[0].shape, target[0].shape, len(data), target[0].max())
            ...
            # assert 1<0, (output[0].shape, target[0].shape)
            losses = [torch.mean(loss_value) for loss_value in losses] 
            loss_val = losses[0] # Collect first loss for weight update
            total_loss += loss_val.item()
            loss_values = [v.item() for v in losses]
            ...
            # gather loss_labels, direct return leads to recursion limit error as it looks for variables to gather'
            loss_labels = list(model.module.loss.loss_labels)
            # assert 1<0, (loss_labels,  loss_values)
        ...
        # progress.close()
        print('Inference Averages ' + tools.format_dictionary_of_losses(loss_labels, np.array(statistics).mean(axis=0)))
    ...
    for epoch in range(args.start_epoch, args.total_epochs + 1):
        ...
        if not args.skip_training:
            ...
            print('Epoch %d/%d : loss: %.4f' % (epoch, args.total_epochs+1, train_loss))
        ...
        if not args.skip_validation and ( (epoch - 1) % args.validation_frequency == 0 
            ...
            if validation_loss < best_err:
                print('Obtain better valid loss : %.4f |--> %.4f' % (best_err, validation_loss))
                ...
                print('best valid loss unchange: %.4f ' % (best_err))
